...
def make_op(tmp_path: pytest.TempPathFactory, **kwargs: object) -> ConvertOperation:
...
class TestConvertOperationValidation:
    ...
    def test_invalid_output_format_raises(self, tmp_path: pytest.TempPathFactory) -> None:
        with pytest.raises(ValidationError):
            ConvertOperation(type="convert", input=str(tmp_path), output_format="avi")

    def test_id_is_auto_generated(self, tmp_path: pytest.TempPathFactory) -> None:
        op1 = make_op(tmp_path)
        op2 = make_op(tmp_path)
        assert op1.id != op2.id

    def test_explicit_id_is_preserved(self, tmp_path: pytest.TempPathFactory) -> None:
        op = make_op(tmp_path, id="my-id")
        assert op.id == "my-id"

    def test_name_defaults_to_none(self, tmp_path: pytest.TempPathFactory) -> None:
        op = make_op(tmp_path)
...
class TestConvertOperationRun:
    ...
    def test_run_calls_ffmpeg_with_correct_args(
        ...
        expected_output = str(input_file.with_suffix(".mp3"))
